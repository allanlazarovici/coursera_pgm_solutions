...
possCopyFactors = IndexToAssignment(1:numAlleles*numAlleles, [numAlleles, \
							    numAlleles]);

for i=1:size(possCopyFactors, 1)
  v1 = possCopyFactors(i, 1);
  v2 = possCopyFactors(i, 2);
  prob = alphaList(allelesToGenotypes(v1, v2));
  offset = 2*numAlleles*(v2 - 1) + 2*(v1 - 1);
