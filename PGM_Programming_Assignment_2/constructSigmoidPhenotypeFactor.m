...
%This question is kind of boring, so I didn't bother with coming
%up with nice syntax. I just want to get this assignment over with.
assignments = IndexToAssignment(1:num^4, [num, num, num, num]);
for i = 1:size(assignments, 1)
  gene1 = assignments(i, [1 3]);
  gene2 = assignments(i, [2 4]);

  prob = computeSigmoid(sum(alleleWeights{1}(gene1)) + \
			sum(alleleWeights{2}(gene2)));

  offset = 2*(i-1);
  phenotypeFactor.val(offset+ 1) = prob;
  phenotypeFactor.val(offset+ 2) = 1 - prob;
end
