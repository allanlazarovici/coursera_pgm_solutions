...
possParentGenotypes = IndexToAssignment(1:numGenotypes^2, [numGenotypes, numGenotypes]);

%I would have liked to avoid using a for loop and use something like apply in R
%However, I couln't figure out the Octave equivalent
for i = 1:size(possParentGenotypes, 1) %Parents genotype is fixed
  offset = (possParentGenotypes(i,2) - 1)*numGenotypes^2 + ...
	   (possParentGenotypes(i,1) - 1)*numGenotypes;

  %Determine the possible genotypes for the child, given parents genotypes
  genoPar1 = possParentGenotypes(i, 1);
  genoPar2 = possParentGenotypes(i, 2);
  [X, Y] = meshgrid(genotypesToAlleles(genoPar1,:), genotypesToAlleles(genoPar2, :));
  possGenos = [X(:), Y(:)];

  for j = 1:size(possGenos, 1)
    alleleA = possGenos(j, 1);
    alleleB = possGenos(j, 2);
    genotypeFactor.val(offset + allelesToGenotypes(alleleA, alleleB)) += 1;
  end  
end
genotypeFactor.val /= 4;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
